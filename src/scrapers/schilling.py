...
class SchillingScraper(BaseOpticalScraper):
    ...
    def __init__(self):
        super().__init__(store_name="schilling", base_url="https://schilling.cl")
        ...
        async with await self.get_client() as client:
            for page in range(1, limit_pages + 1):
                ...
                try:
                    ...
                    logger.error(f"Error scraping Schilling page {page}: {e}")
                    break
    ...
    def _parse_product(self, prod: dict) -> Optional[ScrapedItem]:
        try:
            ...
            brand = prod.get("brand", "Schilling")
            link = prod.get("link", "")
            ...
            if title_el and price_el and link_el:
                ...
                if price:
                    yield ScrapedItem(
                        ...
                        brand=name.split()[0] if name else "Schilling",
                        model_name=name,
                        category=detect_category(name),
                        url=href,
                        price_normal=price,
